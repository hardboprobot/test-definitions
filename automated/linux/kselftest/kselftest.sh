...
SKIPFILE_YAML=""
# Kselftest collections and tests
KSELFTEST_COLLECTIONS=""
KSELFTEST_TESTS=""
BOARD=""
BRANCH=""
ENVIRONMENT=""
...
usage() {
    echo "Usage: $0 [-t kselftest_aarch64.tar.gz | kselftest_armhf.tar.gz]
                    ...
                    [-g branch]
                    [-e environment]
                    [-C Kselftest collections]
                    [-T Kselftest tests]" 1>&2
    exit 1
}
...
while getopts "t:s:u:p:L:S:b:g:e:C:T:h" opt; do
    case "${opt}" in
        ...
        e)
            ...
            ;;
        C)
            export KSELFTEST_COLLECTIONS="${OPTARG}"
            ;;
        T)
            export KSELFTEST_TESTS="${OPTARG}"
            ;;
        h|*) usage ;;
    esac
done
...
if [ "${KSELFTEST_TESTS}" != "None" ]; then
    ./run_kselftest.sh -t "${KSELFTEST_TESTS}" 2>&1 | tee "${LOGFILE}"
elif [ "${KSELFTEST_COLLECTIONS}"  != "None" ]; then
    ./run_kselftest.sh -c "${KSELFTEST_COLLECTIONS}" 2>&1 | tee "${LOGFILE}"
else
